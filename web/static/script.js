...
let colorCardMode = false; // 色卡模式：false=16进制模式，true=色卡模式
let colorPickerModal = null; // 色号选择器弹窗
...
function displayResult(data) {
    ...
    console.log('📦 colorCodes 样例:', data.colorCodes ? data.colorCodes[0]?.[0] : 'undefined');
    ...
    // 更新 currentData，确保后续操作使用正确的颜色数据
    if (!data.colors) {
        console.error('错误：没有颜色数据', data);
        alert('处理失败：服务器返回的数据不完整');
        resetUpload();
        return;
    }
    
    // 绘制画布（直接传入 data，让 drawCanvas 根据模式判断）
    drawCanvas(data);
...
function toggleColorCardMode() {
    ...
    // 重新渲染画布和调色板（直接传入 currentData，让 drawCanvas 自己判断）
...
function showColorPicker(row, col, top3Options, event) {
    ...
    top3Options.forEach((option, index) => {
        ...
        deltaE.textContent = `ΔE: ${option.delta_e}`;
...
function applyColorCodeToCell(row, col, option) {
    ...
    console.log(`🔄 批量替换色号: ${oldCode} → ${newCode}`);
    ...
    console.log(`✅ 共替换了 ${replacedCount} 个像素`);
    ...
    // 只在色卡模式下重绘
    if (colorCardMode) {
        // 使用 requestAnimationFrame 确保一次性完成绘制，避免闪烁
...
function rebuildPaletteFromColorCodes() {
    ...
    console.log(`📊 重建调色板: ${Object.keys(palette).length} 种色号`);
}
...
function displayColorStats(data) {
    ...
    console.log('📊 displayColorStats called, colorCardMode:', colorCardMode);
    ...
    console.log('📊 Total items added:', container.children.length);
...
function getSelectedColors() {
    ...
    // 如果没有保存的数据，返回空数组（表示全选）
    // 注意：空数组在后端会被解释为"使用所有色号"
    return [];
}
